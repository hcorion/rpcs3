...
void patch_manager_dialog::populate_tree()
{
			...
			for (const auto& [title, serials] : patch.titles)
			{
				...
				for (const auto& [serial, app_versions] : serials)
				{
					...
					for (const auto& [app_version, enabled] : app_versions)
					{
						...
						const auto match_criteria = QList<QPair<int, QVariant>>() << QPair<int, QVariant>(description_role, q_description) << QPair<int, QVariant>(persistance_role, true);
	...
	const auto match_criteria = QList<QPair<int, QVariant>>() << QPair<int, QVariant>(persistance_role, true);
