...
* [Qt 6.3.2](https://www.qt.io/download-qt-installer)
...
**Either add the** `QTDIR` **environment variable, e.g.** `<QtInstallFolder>\6.3.2\msvc2019_64\` **, or use the [Visual Studio Qt Plugin](https://marketplace.visualstudio.com/items?itemName=TheQtCompany.QtVisualStudioTools2019)**
...
```
. /etc/os-release
sudo add-apt-repository ppa:beineri/opt-qt-6.3.2-$UBUNTU_CODENAME
sudo apt-get update
sudo apt-get install qt63base qt63svg
. /opt/qt63/bin/qt63-env.sh >/dev/null 2>&1
```
...
2) Add the path to your Qt installation with compiler e.g. `<QtInstallFolder>\6.3\msvc2019_64`, version will fill in automatically.
